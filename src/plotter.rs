...
use crate::data::maxValue;
use crate::utils::Convert::fromTimestamp2Date;
...
pub fn draw(
    ...
    let dataMaxValue = maxValue(&data.iter().map(|tick| tick.high).collect());
    ...
    let maxValue2MaxBalanceRatio =
        dataMaxValue / maxValue(&balanceHistory.iter().map(|balance| balance.1).collect());
