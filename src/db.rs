...
impl Database {
    ...
    pub fn writePattern(&mut self, hash: u64, action: &crate::trader::Actions) -> Result<()> {
        let actionBytes: [u8; 1];
        match action {
            crate::trader::Actions::Buy => actionBytes = (0x00 as u8).to_be_bytes(),
            crate::trader::Actions::Sell => actionBytes = (0x01 as u8).to_be_bytes(),
            crate::trader::Actions::Hold => actionBytes = (0x02 as u8).to_be_bytes(),
        }
        let _ = self
            .db
            .as_ref()
